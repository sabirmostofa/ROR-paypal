...
require 'profile'
require 'caller'
# Controller with actions for doing DoCapture API call. The name is chosen in consistent with other PayPal SDKs.
class DocaptureController < ApplicationController
...
# DoCapture API call  
  def do_capture     
    ...
    @transaction = @caller.call(
      { :method          => 'DoCapture',
        ...
        :trxtype         => 'D',
        :completetype    => params[:CompleteCodeType],
        :amt             => params[:amount],
   ...
   if @transaction.success?       
      session[:capture_response]=@transaction.response
      redirect_to :controller => 'docapture',:action => 'thanks'
    else
  ...
  def thanks
	  @response = session[:capture_response]
  end
